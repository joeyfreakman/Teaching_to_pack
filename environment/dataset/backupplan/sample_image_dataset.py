            ...
            with FileLock(cache_lock_path):
                if not os.path.exists(cache_zarr_path):
                    ...
                    try:
                        ...
                        # print("ReplayBuffer structure after loading:")
                        # print(f"Backend: {replay_buffer.backend}")
                        # print("Root structure:")
                        # for key in replay_buffer.root.keys():
                        #     print(f"  {key}:")
                        #     if isinstance(replay_buffer.root[key], zarr.hierarchy.Group):
                        #         for subkey,subvalue in replay_buffer.root[key].items():
                        #             print(f"subvalue:{subvalue},{subkey}: {type(replay_buffer.root[key][subkey])}")
                        #     else:
                        #         print(f"    {type(replay_buffer.root[key])}")
...
if __name__ == "__main__":
    ...
    num_episodes = 3  # Just to sample from the first 50 episodes for testing
