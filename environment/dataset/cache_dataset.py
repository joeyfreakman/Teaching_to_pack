...
class CacheImageDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        max_start = min(local_index, len(data["actions"]) - self.obs_horizon)
        ...
        action_sequence = data["actions"][max_start:max_start + self.max_len]
    ...
    def load_hdf5(dataset_dir, dataset_name):
        ...
        if compressed:
            ...
            for cam_name in image_dict.keys():
                image_dict[cam_name] = np.array([cv2.cvtColor(img, cv2.COLOR_BGR2RGB) for img in image_dict[cam_name]])

        return image_dict, action_data, compressed
...
if __name__ == "__main__":
    ...
    stats = CacheImageDataset.get_norm_stats([args.dataset_dir], [num_episodes])
