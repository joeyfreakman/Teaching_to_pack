...
from src.model.ema_model import EMAModel
from src.model.util import set_seed, detach_dict, compute_dict_mean
from environment.dataset.curr_image_dataset import load_merged_data
...
def train_ddpm(train_dataloader, val_dataloader, pretest_dataloader, config):
    ...
    if os.path.isdir(ckpt_dir) and len(os.listdir(ckpt_dir)) > 2:
        ...
        if load_ckpt == "y":
            ...
            if "ema_state_dict" in checkpoint:
                ema_model.averaged_model.load_state_dict(checkpoint["ema_state_dict"])
    ...
    for epoch in tqdm(range(start_epoch, num_epochs)):
        ...
        with torch.no_grad():
            for batch in tqdm(val_dataloader):
                try:
                    forward_dict = forward_pass(batch, ema_model.averaged_model)
                    loss = forward_dict["loss"]
                    val_loss += loss.item()
                except:
                    if 'out of memory' in str(e):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(ckpt_dir, f"best_model.pth")
            last_best_model_path = best_model_path
            torch.save(ema_model.averaged_model.state_dict(), best_model_path)
    ...
    # test
    ema_model.averaged_model.load_state_dict(torch.load('best_model.pth'))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--max_len', action='store', type=int, help='max_len', default=20, required=False)
    #diffusion
